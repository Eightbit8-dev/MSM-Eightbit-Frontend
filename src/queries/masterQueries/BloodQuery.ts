  ...
  const fetchAllBloods = async (): Promise<BloodDetails[]> => {
    try {
      ...
      if (axios.isAxiosError(error)) {
        toast.error(error.response?.data?.message || "Failed to fetch Bloods");
      } else {
...
export const useFetchBloodOptions = () => {
  const fetchOptions = async (): Promise<DropdownOption[]> => {
    ...
    return res.data.map((blood: BloodDetails) => ({
      id: blood.id,
      label: blood.name,
    }));
  };
...
export const useDeleteBlood = () => {
  ...
  const deleteBlood = async (Blood: BloodDetails) => {
    ...
    const res = await axiosInstance.delete(`${apiRoutes.Bloods}/${Blood.id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
